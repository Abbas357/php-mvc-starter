...
if(isset($_POST['deleteMsg']) && !empty($_POST['deleteMsg'])){
	...
	$messageClass->deleteMsg($messageID, $user_id);
}

if(isset($_POST['sendMessage']) && !empty($_POST['sendMessage'])){
	$user_id = $_SESSION['user_id'];
	$message = $userClass->checkInput($_POST['sendMessage']);
	$get_id = $_POST['get_id'];
	if(!empty($message)){
		$userClass->create('messages', array('messageTo' => $get_id, 'messageFrom' => $user_id, 'message' => $message, 'messageOn' => date('Y-m-d H:i:s')));
	}
}
...
if(isset($_POST['showChatMessage']) && !empty($_POST['showChatMessage'])){
	...
	$messages = $messageClass->getMessages($messageFrom, $user_id);
	//Notification
	$messageClass->messagesViewed($user_id);
}
if(isset($_POST['showMessage']) && !empty($_POST['showMessage'])){
	$user_id = $_SESSION['user_id'];
	$messages = $messageClass->recentMessages($user_id);
	?>
							...
							<div class="people-message" data-user="<?php echo $message->user_id; ?>">
								<div class="people-inner">
									...
									<div class="name-right2">
										<span><a href="#"><?php echo $message->screenName; ?></a><?php echo $userClass->checkVerified($message->user_id) ? "<i class=\"icon-verified verified\"></i>" : ""; ?></span><span> @<?php echo $message->username; ?></span>
									</div>
									
									<span>
										<?php echo $userClass->timeAgo($message->messageOn); ?>
									</span>
								</div>
							</div>
							<!--Direct Messages-->
...
if(isset($_POST['showChatPopup']) && !empty($_POST['showChatPopup'])){
	...
	$user_id = $_SESSION['user_id'];
	$user = $userClass->userData($messageFrom);
	?>
		...
		<div class="wrap3">
			<div class="message-send2">
				...
				<div class="main-msg-wrap">
					<div class="main-msg-inner" id="chat">
				...
				<div class="main-msg-footer">
					<div class="main-msg-footer-inner">
						<ul>
							<li><textarea id="msg" name="msg" placeholder="Type a message"></textarea></li>
