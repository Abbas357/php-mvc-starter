...
if(isset($_POST['hashtag'])){
	$hashtag = $userClass->checkInput($_POST['hashtag']);
	$mention = $userClass->checkInput($_POST['hashtag']);

	if(substr($hashtag,0,1) === '#'){
		$trend = str_replace('#', '', $hashtag);
		$trends = $postClass->getTrendByHash($trend);
	...
	if(substr($mention,0,1) === '@'){
		...
		$mentions = $postClass->getMention($mention);
